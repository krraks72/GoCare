 ...
 * @var iterable<\App\Model\Entity\Fantecedente> $fantecedentes
 */
?>
<div class="row">
    <aside class="column">
    ...
    <div class="column-responsive column-80">
        <div class="revision form content">
            ...
            <h3><?= __('Antecedentes - Terapias') ?></h3>
            <div class="table-responsive">
                <table>
                    ...
                    <tbody>
                        <?php foreach ($fantecedentes as $fantecedente): ?>
                        <tr>
                            <td><?= h($fantecedente->registro->id) ?></td>
                            <td><?= h($fantecedente->fecha) ?></td>
                            <td><?= h($fantecedente->valor) ?></td>
                            <td><?= $fantecedente->has('tipo_antecedente') ? $this->Html->link($fantecedente->tipo_antecedente->descripcion, ['controller' => 'TipoAntecedentes', 'action' => 'view', $fantecedente->tipo_antecedente->id]) : '' ?></td>
                            <td class="actions">
                                <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $fantecedente->id, $consecutivo], ['confirm' => __('Are you sure you want to delete # {0}?', $fantecedente->id)]) ?>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
            ...
            <?= $this->Html->link(__('Siguiente'), ['controller' => 'frevisiones', 'action' => 'index', $consecutivo],['class' => 'button']) ?>
        </div>
    </div>
</div>
